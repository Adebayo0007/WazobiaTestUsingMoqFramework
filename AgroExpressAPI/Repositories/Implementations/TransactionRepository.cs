...
namespace AgroExpressAPI.Repositories.Implementations;
    ...
    public async Task<IEnumerable<Transaction>> GetAllAsync() =>
        await _applicationDbContext.Transactions.ToListAsync();
    public async Task<IEnumerable<Transaction>> GetByEmailAsync(string userEmail) =>
        await _applicationDbContext.Transactions.Where(t => t.BuyerEmail == userEmail || t.FarmerEmail == userEmail).ToListAsync();

    public Transaction GetByReferenceNumberAsync(string refNumber) =>
       _applicationDbContext.Transactions.SingleOrDefault(t => t.ReferenceNumber == refNumber);
